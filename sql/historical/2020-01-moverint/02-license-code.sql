...
ALTER TABLE loss.contribution 
  ADD COLUMN license_code VARCHAR REFERENCES cf_common.license(code);

UPDATE loss.contribution c
   SET license_code=l.code
  FROM cf_common.license l 
 WHERE c.license_id=l.id;

ALTER TABLE loss.contribution ALTER COLUMN license_code SET NOT NULL;
ALTER TABLE loss.contribution DROP COLUMN license_id;
